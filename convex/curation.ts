...
import {
  visibility, itemTarget, followTarget, listDocument,
  communityDocument, membershipDocument, followDocument, membershipStatus,
...
// No repository ACL exists yet. Never infer private access from authentication.
// Check quoted/source content too, with a hard depth limit that fails closed.
...
async function targetDetails(ctx: QueryCtx, target: Infer<typeof itemTarget>) {
  ...
  if (target.kind === "repo") {
    ...
    return {
      ...
      targetHref: `/repos/${encodeURIComponent(repository.ownerLogin)}/${encodeURIComponent(repository.name)}`,
    };
  }
  ...
  return {
    targetLabel: `${profile ? `@${profile.handle}` : "OpenHub developer"} · ${post.type}`,
    targetHref: `/posts/${post._id}`,
  };
}
...
export const listItems = query({
  args: { listId: v.id("lists"), paginationOpts: paginationOptsValidator }, returns: pageValidator(listItemView),
  handler: async (ctx, args) => {
    ...
    const visible = await Promise.all(result.page.map(item => targetDetails(ctx, item.target)));
    // Post-pagination checks keep reads bounded and hide targets made private later.
    return { ...pageResult(result), page: result.page.flatMap((item, index) => visible[index] ? [{ ...item, ...visible[index] }] : []) };
  },
});
