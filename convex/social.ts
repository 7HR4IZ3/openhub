...
import { canInteractWithPost, canViewPostWithContext } from "./post-access";
...
export const toggleLike = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const post = await ctx.db.get(args.postId);
    if (post === null || !await canInteractWithPost(ctx, post, userId) || await isPostSuppressed(ctx, post, userId)) {
...
export const toggleRepost = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const post = await ctx.db.get(args.postId);
    if (post === null || post.visibility !== "public" || await isPostSuppressed(ctx, post, userId)) {
