...
import { RecentPostFeed, type PostFeedMode } from "@/components/posts/recent-post-feed";
      ...
      <div className="mx-auto grid min-h-screen w-full max-w-[1440px] lg:grid-cols-[232px_minmax(0,1fr)] xl:grid-cols-[232px_minmax(0,680px)_304px]">
        ...
        <main className="min-h-screen min-w-0 border-x border-black/[0.08] bg-[#fbfbf9] pb-24 dark:border-white/[0.08] dark:bg-[#151714] lg:pb-0">
          ...
          {convexConfigured && activeTab === "For you" ? (
            <section className="border-b border-black/[0.08] p-5 dark:border-white/[0.08] lg:p-7" aria-label="Repository recommendations">
              <RecommendationsPanel convexConfigured={convexConfigured} />
            </section>
          ) : null}
          <p role="status" className="border-b px-5 py-4 text-sm text-muted-foreground lg:px-7">
            {convexConfigured
              ? activeTab === "For you"
                ? "Recommendations stay explainable: source context, learning value, and a clear reason to open."
                : activeTab === "Following"
                  ? "Your following trail is built from people, repositories, and topics you chose."
                  : "Trending is weighted toward fresh, constructive technical conversation—not raw volume."
              : "Connect Convex to make your following trail, recommendations, and social feed persistent."}
          </p>
...
function feedMode(tab: (typeof tabs)[number]): PostFeedMode {
