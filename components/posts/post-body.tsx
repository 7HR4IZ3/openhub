    ...
    <div className="space-y-4 text-[1.02rem] leading-8 text-foreground/90">
      {segments.map((segment, index) => {
              ...
              {segment.split(/\r?\n/).map((line, lineIndex) => {
                ...
                if (line.startsWith("> "))
                  return (
                    <blockquote
                      key={`${index}-${lineIndex}`}
                      className="border-l-2 border-ring pl-4 text-muted-foreground"
                    >
                      {renderInline(
                        line.slice(2),
                        `${index}-${lineIndex}`,
                ...
                return (
                  <p key={`${index}-${lineIndex}`}>
                    {renderInline(line, `${index}-${lineIndex}`, lineLinkBase)}
                  </p>
                );
              })}
            </div>
        ...
        return (
          <div
            key={`code-${index}`}
            className="overflow-hidden rounded-xl border border-border bg-muted dark:bg-muted"
          >
            {language ? (
...
function renderInline(
  ...
  const token =
    /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)|(@[a-zA-Z0-9][a-zA-Z0-9_-]{1,39})|(#L\d+(?:-L?\d+)?)/g;
  ...
  while ((match = token.exec(value)) !== null) {
    ...
    if (match[1] && match[2]) {
      ...
      const href = lineLinkBase
        ? lineLinkBase.replace(/#.*$/, "") + lineAnchor(lineReference)
        : null;
      nodes.push(
        href ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="rounded bg-secondary px-1 font-mono text-[0.9em] text-foreground underline-offset-2 hover:underline dark:bg-secondary"
          >
            {lineReference}
          </a>
        ) : (
          <span
            key={`${keyPrefix}-line-${index}`}
            className="rounded bg-secondary px-1 font-mono text-[0.9em] dark:bg-secondary"
          >
            {lineReference}
          </span>
        ),
