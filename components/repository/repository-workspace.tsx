...
function ConnectedRepositoryWorkspace({ repository, convexConfigured, ...props }: RepositoryWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    void track({ eventName: "repository_view", repositoryId: storedRepositoryId });
...
function RepositoryWorkspaceView({
  ...
  const discussionHref = file
    ? repository.provider === "github" && repository.visibility === "public"
      ...
      : null
    : null;
  const diffHref = file && repository.provider === "github" && surfaces.commits.length > 1
        ...
        <div className="mx-auto max-w-[1520px] px-4 py-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between gap-4 text-sm">
            ...
            <div className="flex items-center gap-3">
              <span className="hidden items-center gap-1.5 text-xs text-muted-foreground sm:inline-flex">
                ...
                Live from {providerDisplayName(repository.provider)}
              </span>
          ...
          <div className="mt-7 flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
            ...
            <div className="flex shrink-0 flex-wrap gap-2">
              ...
              {repository.visibility === "public" && repository.provider === "github" ? <>
                <RepositoryFollowButton repository={repository} convexConfigured={convexConfigured} />
                <RepositorySaveButton repository={repository} convexConfigured={convexConfigured} />
              </> : <span className="inline-flex items-center gap-2 rounded-full border border-black/[0.1] px-4 py-2 text-xs font-semibold text-muted-foreground dark:border-white/[0.1]"><LockKeyhole className="h-3.5 w-3.5" /> {repository.visibility === "private" ? "Private access" : `${providerDisplayName(repository.provider)} browse`}</span>}
            </div>
          </div>
                ...
                {file ? (
                  <>
                    ...
                    {convexConfigured && repository.provider === "github" && repository.visibility === "public" ? <Button
                      ...
                      title={selection === null ? "Select lines to explain" : "Explain selected lines"}
                    >
                ...
                <div className="flex flex-col gap-3 border-t border-black/[0.08] px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 dark:border-white/[0.08]">
                  ...
                  {discussionHref ? (
                    <div className="flex flex-wrap gap-2">
                      ...
                      {diffHref ? <Button asChild size="sm" variant="outline" className="shrink-0 rounded-full bg-transparent"><Link href={diffHref}><GitCompareArrows className="h-3.5 w-3.5" />Discuss diff</Link></Button> : null}
                    </div>
                  ) : null}
                </div>
...
function ConnectedRepositoryFollowButton({ repository }: { repository: NormalizedRepository }) {
  ...
  async function toggle() {
    ...
    try {
      if (following) {
        if (!storedRepositoryId) throw new Error("Repository is not ready to follow");
        await setFollow({ target: { kind: "repo", repositoryId: storedRepositoryId }, following: false });
      } else {
        const repositoryId: Id<"repositories"> = storedRepositoryId ?? await observe({ owner: repository.ownerLogin, name: repository.name });
        await setFollow({ target: { kind: "repo", repositoryId }, following: true });
      }
    } catch (actionError) {
      setError(actionError instanceof Error ? actionError.message : "Repository follow could not be saved");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <Button type="button" className="rounded-full" variant={following ? "secondary" : "default"} onClick={() => void toggle()} disabled={pending}>
        <Heart className="h-4 w-4" />
        {pending ? "Saving…" : following ? "Following" : "Follow"}
      </Button>
      {error ? <span className="max-w-48 text-right text-[10px] leading-4 text-destructive">{error}</span> : null}
    </div>
  );
}
...
function RepositorySaveButton({
  ...
  if (!convexConfigured) {
    return (
      <Button asChild variant="outline" className="rounded-full bg-transparent">
        <Link href="/signin"><Bookmark className="h-4 w-4" /> Save</Link>
      </Button>
    );
  }
...
function ConnectedRepositorySaveButton({ repository }: { repository: NormalizedRepository }) {
  const { isAuthenticated } = useConvexAuth();
  const repositoryId = useQuery(api.discovery.byProviderRepository, {
    provider: repository.provider,
    providerRepositoryId: repository.providerRepositoryId,
  });
  const lists = useQuery(api.curation.myListChoices, isAuthenticated ? {} : "skip") as Array<{ _id: Id<"lists">; title: string; visibility: "public" | "private" }> | undefined;
  const observe = useAction(api.discovery.observePublicRepository);
  ...
  async function saveToList(listId: Id<"lists">) {
    ...
    setError(null);
    try {
      const ensuredRepositoryId: Id<"repositories"> = repositoryId ?? await observe({ owner: repository.ownerLogin, name: repository.name });
      await addItem({ listId, target: { kind: "repo", repositoryId: ensuredRepositoryId } });
      setSaved((current) => new Set(current).add(listId));
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : "Repository could not be saved");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <div className="relative">
      <Button type="button" variant="outline" className="rounded-full bg-transparent" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
        <Bookmark className="h-4 w-4" /> Save
      </Button>
      {open ? <div className="absolute right-0 top-full z-20 mt-2 w-64 rounded-2xl border border-black/[0.1] bg-[#fbfbf9] p-3 shadow-sm dark:border-white/[0.1] dark:bg-[#151714]">
        <p className="px-2 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Save to a list</p>
        {lists === undefined ? <p className="px-2 py-3 text-xs text-muted-foreground">Loading lists…</p> : lists.length === 0 ? <Link href="/lists" className="block px-2 py-3 text-xs leading-5 text-muted-foreground hover:text-foreground">Create a list first <ArrowUpRight className="inline h-3.5 w-3.5" /></Link> : <div className="mt-1 max-h-56 overflow-y-auto">{lists.map((list) => <button key={list._id} type="button" className="flex w-full items-center justify-between gap-3 rounded-xl px-2 py-2 text-left text-sm hover:bg-black/[0.05] dark:hover:bg-white/[0.06]" onClick={() => void saveToList(list._id)} disabled={pending}><span className="min-w-0 truncate">{list.title}</span><span className="text-[10px] text-muted-foreground">{saved.has(list._id) ? "Saved" : list.visibility}</span></button>)}</div>}
        {error ? <p role="alert" className="mt-2 px-2 text-[10px] leading-4 text-destructive">{error}</p> : null}
      </div> : null}
    </div>
  );
}
...
function buildSourceUrl(
  ...
  const fullName = repository.fullName.split("/").map(encodeURIComponent).join("/");
  const root = repository.provider === "github"
...
function buildDiffComposeHref(repository: NormalizedRepository, file: RepositoryFile, baseCommitSha: string) {
  const params = new URLSearchParams({
...
function buildExplainableSource(
  ...
  const startLine = Math.max(1, Math.min(selection.startLineNumber, lines.length));
  const endLine = Math.max(startLine, Math.min(selection.endLineNumber, lines.length));
  return {
    provider: repository.provider,
    ...
    ...(repository.licenseSpdxId ? { licenseSpdxId: repository.licenseSpdxId } : {}),
    visibility: "public",
...
function providerDisplayName(provider: NormalizedRepository["provider"]) {
  return provider === "github" ? "GitHub" : provider === "gitlab" ? "GitLab" : provider === "bitbucket" ? "Bitbucket" : "Codeberg";
}
