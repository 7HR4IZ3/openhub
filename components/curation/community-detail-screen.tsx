...
function ConnectedCommunityDetail({
  communityId,
}: {
    ...
    return (
      ...
      >
        <div className="p-5 sm:p-7">
          <CurationLoading label="Loading this circle…" />
        </div>
      </CurationShell>
    );
  if (community === null)
    return (
      ...
      >
        <CurationEmptyState
          ...
          icon={Users}
          eyebrow="Circle unavailable"
          title="This community is private or no longer exists."
          body="Private circles are visible only to active members. Return to public discovery to find another source-backed discussion space."
  ...
  return (
    ...
    >
      <section className="border-b border-border p-5 sm:p-7">
        ...
        >
          <ArrowLeft className="h-3.5 w-3.5" /> All circles
        </Link>
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">
              ...
              {community.visibility === "public" ? (
                <>
                  <Globe2 className="h-3.5 w-3.5" /> Public circle
                </>
              ) : (
                <>
                  <LockKeyhole className="h-3.5 w-3.5" /> Private circle
                </>
              )}
            </div>
      ...
      <section className="p-5 sm:p-7">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {active ? (
              <Button asChild className="rounded-md">
                <Link
                  href={`/compose?communityId=${encodeURIComponent(communityId)}`}
                >
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <Signal
            label="Visibility"
            value={community.visibility === "public" ? "Public" : "Private"}
          />
          <Signal label="Membership" value={active ? "Active" : "Not joined"} />
          <Signal label="Posts" value={String(posts.results.length)} />
        </div>
        <div className="mt-10 flex items-end justify-between gap-4">
          ...
          {active ? (
            ...
            >
              Write <ArrowUpRight className="inline h-3.5 w-3.5" />
            </Link>
          ) : null}
        </div>
        ...
        {posts.results.length === 0 && posts.status !== "LoadingFirstPage" ? (
          <CurationEmptyState
            className="mt-5"
            icon={Code2}
            eyebrow="No community posts"
            title="Start with one useful question."
            body="A good first post names the source, the tradeoff, or the thing you are trying to understand."
            action={active ? "Write a post" : "Join the circle"}
            actionHref={
              active
                ? `/compose?communityId=${encodeURIComponent(communityId)}`
                : undefined
            }
          />
        ) : null}
