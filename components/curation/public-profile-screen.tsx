...
} from "@/components/curation/profile-screen";
    ...
    return (
      ...
      >
        <div className="p-5 sm:p-7">
          <CurationLoading label="Loading profile…" />
        </div>
      </CurationShell>
    );
  if (profile === null)
    return (
      ...
      >
        <CurationEmptyState
          ...
          title="This developer profile could not be found."
          body="The handle may have changed or the profile may not be connected yet."
...
function ConnectedPublicProfile({ profile }: { profile: Doc<"profiles"> }) {
  ...
  const following = useQuery(api.curation.isFollowing, {
  ...
  const repositories = useQuery(
    api.profiles.repositories,
    profile.githubLogin
      ? { githubLogin: profile.githubLogin, limit: 12 }
      : "skip",
  );
  ...
  return (
    ...
    >
      <section className="border-b border-border p-5 sm:p-7">
        ...
        <div className="mt-5 flex flex-wrap gap-2 text-xs text-muted-foreground">
          {profile.interests.map((interest) => (
            <span
              key={interest}
              className="rounded-full bg-secondary px-3 py-1.5 dark:bg-secondary"
            >
              {interest}
            </span>
          ))}
      ...
      <section
        ...
        aria-labelledby="public-repositories-heading"
      >
        <div className="flex items-end justify-between gap-3">
          <div>
            ...
            <h2
              id="public-repositories-heading"
          ...
          <span className="text-xs text-muted-foreground">
            {repositories?.length ?? 0} shown
          </span>
        </div>
        {repositories === undefined ? (
          <div className="mt-5">
            <CurationLoading label="Loading public repositories…" />
          </div>
        ) : repositories.length === 0 ? (
          <p className="mt-5 text-sm leading-6 text-muted-foreground">
            No public repositories have been imported for this profile yet.
          </p>
        ) : (
      ...
      <section className="p-5 sm:p-7" aria-labelledby="public-posts-heading">
        <div className="flex items-end justify-between gap-3">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">
        ...
        {posts.results.length === 0 && posts.status !== "LoadingFirstPage" ? (
          <CurationEmptyState
            ...
            title="This trail has not started here yet."
            body="Explore the repositories this developer studies, then return when the first useful context is published."
            action="Explore repositories"
            actionHref="/explore"
          />
        ) : null}
        ...
        {posts.status === "CanLoadMore" || posts.status === "LoadingMore" ? (
          <div className="mt-5 text-center">
            <Button
              ...
              onClick={() => posts.loadMore(12)}
              disabled={posts.status === "LoadingMore"}
            >
              {posts.status === "LoadingMore" ? "Loading…" : "Load more posts"}
            </Button>
          </div>
        ) : null}
      </section>
...
function Avatar({ profile }: { profile: Doc<"profiles"> }) {
  const initials = profile.displayName
    ...
    .toUpperCase();
  return (
    <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-[1.25rem] bg-secondary text-lg font-semibold dark:bg-secondary">
      {profile.avatarUrl ? (
        <Image
