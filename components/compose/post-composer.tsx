...
const postTypes: Array<{ value: PostType; label: string; hint: string }> = [
  ...
  { value: "task", label: "Task", hint: "Point people toward a concrete technical task" },
  { value: "bounty", label: "Bounty", hint: "Share a task with an external reward or issue link" },
...
export function PostComposer({
  ...
  if (convexConfigured) {
    return <ConnectedPostComposer source={source} diff={diff} sourceError={sourceError} communityId={communityId} />;
  }
...
function ConnectedPostComposer({
  ...
  async function publish(input: CreatePostInput) {
    ...
    try {
      const verifiedCommunityId = input.communityId as Id<"communities"> | undefined;
      const post = input.sourceReference
        ? await createSource({ type: input.type, body: input.body, visibility: input.visibility, communityId: verifiedCommunityId, sourceReference: input.sourceReference })
        : input.diffReference
          ? await createDiff({ type: input.type, body: input.body, visibility: input.visibility, communityId: verifiedCommunityId, diffReference: input.diffReference })
          : await createPost({ type: input.type, body: input.body, visibility: input.visibility, ...(verifiedCommunityId === undefined ? {} : { communityId: verifiedCommunityId }) });
        ...
        <div className="mx-auto max-w-5xl py-12 sm:py-16">
          {source !== null ? (
            <p role="status" className="mb-6 rounded-md border p-4 text-sm">
              The source will be verified against its exact public commit before it is published.
          ...
          {communityId ? <p className="mt-6 inline-flex rounded-full bg-[#e9e6dc] px-3 py-2 text-xs font-semibold text-[#8f432b] dark:bg-[#20251f] dark:text-[#f0b38d]">Publishing inside a community</p> : null}
            ...
            <section className="min-w-0 rounded-[1.6rem] border border-black/[0.1] bg-[#fbfbf9] p-5 dark:border-white/[0.1] dark:bg-[#151714] sm:p-7">
              ...
              {sourceError ? (
                <div className="mt-6 rounded-2xl border border-[#b45e3c]/30 bg-[#b45e3c]/[0.06] p-4 text-sm leading-6">
                  <p className="font-semibold">The code context could not be attached.</p>
...
function DiffAttachment({ diff }: { diff: DiffDisplayContext }) {
  return (
    <div className="mt-7 overflow-hidden rounded-2xl border border-black/[0.1] dark:border-white/[0.1]">
      <div className="flex flex-col gap-3 border-b border-black/[0.08] px-4 py-3 dark:border-white/[0.08] sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0"><div className="flex items-center gap-2 text-sm font-semibold"><FileCode2 className="h-4 w-4 shrink-0 text-[#b45e3c] dark:text-[#e99970]" /><span className="truncate font-mono">{diff.path}</span></div><p className="mt-1 truncate pl-6 text-xs text-muted-foreground">{diff.repositoryFullName} · {shortSha(diff.baseCommitSha)} → {shortSha(diff.headCommitSha)}</p></div>
      ...
      <div className="border-t border-black/[0.08] px-4 py-3 text-xs leading-5 text-muted-foreground dark:border-white/[0.08]">Both snapshots are read-only and will be rechecked at publish time. Original owner: @{diff.originalOwner}{diff.licenseSpdxId ? ` · ${diff.licenseSpdxId}` : ""}.</div>
    </div>
  );
}
