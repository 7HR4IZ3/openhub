...
export function RepositoryAnalyzer({
  ...
  async function submit() {
    ...
    setError(null);
    try {
      setAnswer(await analyze({ owner, name, ref: sourceRef, mode, ...(question.trim() ? { question: question.trim() } : {}) }));
    } catch (analysisError) {
      setError(analysisError instanceof Error ? analysisError.message : "Repository analysis could not be generated");
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <section className="rounded-2xl border border-[#b45e3c]/30 bg-[#b45e3c]/[0.04] p-5 dark:border-[#e99970]/30 dark:bg-[#e99970]/[0.05]" aria-labelledby="repository-analysis-heading">
      ...
      {!convexConfigured ? <p className="mt-4 text-xs leading-5 text-muted-foreground">Connect Convex before repository analysis is available.</p> : (
        <>
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            <label className="sr-only" htmlFor="repository-analysis-question">What should OpenHub explain?</label>
            <input id="repository-analysis-question" value={question} onChange={(event) => setQuestion(event.target.value)} maxLength={2_000} placeholder="Where should I start reading?" className="h-9 min-w-0 flex-1 rounded-md border border-black/[0.12] bg-transparent px-3 text-xs outline-none focus-visible:ring-1 focus-visible:ring-[#b45e3c] dark:border-white/[0.12]" />
            <label className="sr-only" htmlFor="repository-analysis-mode">Analysis mode</label>
            <select id="repository-analysis-mode" value={mode} onChange={(event) => setMode(event.target.value as AnalysisMode)} className="h-9 rounded-md border border-black/[0.12] bg-transparent px-3 text-xs dark:border-white/[0.12]">
          ...
          {answer ? (
            <div className="mt-5 border-t border-[#b45e3c]/20 pt-5 dark:border-[#e99970]/20">
              <div className="whitespace-pre-wrap text-sm leading-6 text-foreground/90">{answer.text}</div>
              <div className="mt-5 flex flex-wrap items-center gap-x-3 gap-y-2 text-[10px] text-muted-foreground">
                <span className="inline-flex items-center gap-1"><Check className="h-3.5 w-3.5 text-[#9b4d31] dark:text-[#e99970]" />{answer.filesIncluded} commit-pinned files</span>
