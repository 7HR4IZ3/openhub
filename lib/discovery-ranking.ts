...
export function calculateRepositoryRank(
  ...
  const documentation = evidence.documentation === undefined ? 0
    : (evidence.documentation.descriptionPresent ? 1.5 : 0)
      + (evidence.documentation.homepagePresent ? 1 : 0)
      + (evidence.documentation.wikiEnabled ? 0.5 : 0)
      + (evidence.documentation.issuesEnabled ? 0.5 : 0)
      + (evidence.documentation.discussionsEnabled ? 0.5 : 0);
  const penalty = (evidence.archived ? 40 : 0) + (evidence.isFork ? 5 : 0);
  const score = Math.round(clamp(recentActivity + adoption + relevance + license + documentation - penalty, 100) * 100) / 100;
  return {
    score,
    calculatedAt,
    ...
    explanations: [
      ...
      evidence.documentation === undefined
        ? "Documentation signals are not available for this observation."
        : `Repository context: ${evidence.documentation.descriptionPresent ? "description" : "no description"}, ${evidence.documentation.homepagePresent ? "homepage" : "no homepage"} (+${documentation.toFixed(1)}); provider flags are context, not learning quality.`,
